...
Например:
 ...
 Для управления данными формы и их вводом есть, например, популярное решение - redux-form, которое хранит лишь данные формы. 
 ...
 Для разделения логики и отображения и создан данный модуль. Суть его в том, что помимо данных элемента формы, мы храним и набор параметров, описывающих его визуальные характеристики.
...
- за логику формы отвечают набор элементов и их взаимодействие.

Данные формы (набор элементов и их полей) хранятся в `redux` `store`, поэтому
необходимо подключить их в `props` и подключить метод для их изменения.
...
Для примера выше, установка в объекте с id == `email` поля hidden равным `false`
...
### Подключение к компоненту формы
...
- `kateFormPath` - необязательный параметр.
Путь к данным `kate-form` в `redux` `store`, по умолчанию `'kate-form'`
